    ...
    <div class="breadcrumb-header justify-content-between page-header">
        <div class="my-auto">
            <div class="d-flex">
                ...
                <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ تعديل دور: <strong>{{ $role->name ?? 'محاسب' }}</strong></span>
            </div>
        </div>
...
@section('content')
    ...
    @php
        // Get current role permissions - works with both real $role and dummy data
        $currentPermissions = isset($role) && isset($role->permissions)
                ...
                <div class="card custom-card p-4">
                    <div class="form-group mb-0">
                        ...
                        <input type="text" name="name" class="form-control-modern" required
                               value="{{ old('name', $role->name ?? 'محاسب') }}"
                               placeholder="مثال: مدير المبيعات، محاسب، موظف فواتير...">
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="perm-group-card">
                    ...
                    <div class="perm-group-body">
                        ...
                        @foreach($invoicePerms as $index => $perm)
                            ...
                            <div class="perm-item">
                                <label class="perm-label {{ $isChecked ? 'active' : '' }}" for="perm_inv_{{ $index }}">{{ $perm }}</label>
                                <label class="custom-switch">
                                    <input type="checkbox" name="permission[]" value="{{ $perm }}"
                                           id="perm_inv_{{ $index }}" {{ $isChecked ? 'checked' : '' }}>
                ...
                <div class="perm-group-card">
                    ...
                    <div class="perm-group-body">
                        ...
                        @foreach($archivePerms as $index => $perm)
                            @php $isChecked = in_array($perm, $currentPermissions); @endphp
                            <div class="perm-item">
                                <label class="perm-label {{ $isChecked ? 'active' : '' }}" for="perm_arc_{{ $index }}">{{ $perm }}</label>
                                <label class="custom-switch">
                                    <input type="checkbox" name="permission[]" value="{{ $perm }}"
                                           id="perm_arc_{{ $index }}" {{ $isChecked ? 'checked' : '' }}>
                                    <span class="slider"></span>
                                </label>
                            </div>
                        @endforeach
                ...
                <div class="perm-group-card">
                    ...
                    <div class="perm-group-body">
                        ...
                        @foreach($prodPerms as $index => $perm)
                            @php $isChecked = in_array($perm, $currentPermissions); @endphp
                            <div class="perm-item">
                                <label class="perm-label {{ $isChecked ? 'active' : '' }}" for="perm_prod_{{ $index }}">{{ $perm }}</label>
                                <label class="custom-switch">
                                    <input type="checkbox" name="permission[]" value="{{ $perm }}"
                                           id="perm_prod_{{ $index }}" {{ $isChecked ? 'checked' : '' }}>
                                    <span class="slider"></span>
                                </label>
                            </div>
                        @endforeach
                ...
                <div class="perm-group-card">
                    ...
                    <div class="perm-group-body">
                        ...
                        @foreach($userPerms as $index => $perm)
                            @php $isChecked = in_array($perm, $currentPermissions); @endphp
                            <div class="perm-item">
                                <label class="perm-label {{ $isChecked ? 'active' : '' }}" for="perm_usr_{{ $index }}">{{ $perm }}</label>
                                <label class="custom-switch">
                                    <input type="checkbox" name="permission[]" value="{{ $perm }}"
                                           id="perm_usr_{{ $index }}" {{ $isChecked ? 'checked' : '' }}>
                                    <span class="slider"></span>
                                </label>
                            </div>
                        @endforeach
...
@section('js')
    <script>
        $(document).ready(function() {
            // Sync label color on toggle change
